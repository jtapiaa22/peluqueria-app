    ...
    <div className="page-animation">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14, marginBottom: 24 }}>
        ...
        <div className="card" style={{ textAlign: 'center', margin: 0, border: '1px solid var(--border-primary)' }}>
          ...
          <div style={{ color: 'var(--text-muted)', fontSize: 11, marginTop: 4 }}>{atenciones.length} atenciones</div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
        ...
        <div className="card" style={{ margin: 0 }}>
          ...
          <table className="table">
            <thead>
              <tr><th>Peluquero</th><th>Atenciones</th><th>Total</th><th>Promedio</th></tr>
            </thead>
            <tbody>
              {Object.entries(resumenPorPeluquero)
                ...
                .map(([nombre, data]) => (
                  <tr key={nombre}>
                    ...
                    <td style={{ color: 'var(--text-muted)' }}>${(data.total / data.atenciones).toLocaleString('es-AR', { maximumFractionDigits: 0 })}</td>
                  </tr>
                ))}
              ...
              {Object.entries(resumenPorServicio)
                ...
                .map(([nombre, data]) => (
                  <tr key={nombre}>
                    ...
                    <td>{data.cantidad}</td>
                    <td style={{ color: '#4ade80', fontWeight: 600 }}>${data.total.toLocaleString('es-AR')}</td>
                    <td style={{ color: 'var(--text-muted)' }}>{((data.cantidad / atenciones.length) * 100).toFixed(1)}%</td>
                  </tr>
                ))}
